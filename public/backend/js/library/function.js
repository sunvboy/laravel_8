...
function openKCFinderAlbum(field, type, result) {
    ...
    window.open(
        BASE_URL +
        "plugin/kcfinder-3.12/browse.php?type=" +
        type +
        "&dir=images/public",
        "kcfinder_image",
...
$(document).on("click", ".ajax-delete", function (e) {
    ...
    swal(
        ...
        function (isConfirm) {
            if (isConfirm) {
                let formURL = BASE_URL_AJAX + "ajax-delete";
                $.ajax({
                    headers: {
...
$(document).on('change', '.publish-ajax', function () {
    ...
    $.ajax({
        type: 'POST',
        url: BASE_URL_AJAX + "publish-ajax",
        headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
        data: { param: param },
        success: function (data) {
    ...
    swal({
        ...
        function (isConfirm) {
            if (isConfirm) {
                $.ajax({
                    type: 'POST',
                    url: BASE_URL_AJAX + "ajax-delete-all",
                    headers: { 'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content') },
                    data: { param: param },
                    success: function (data) {
                        if (data.code == 200) {
