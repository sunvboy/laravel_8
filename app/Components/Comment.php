...
class Comment{
    ...
    public function comment($param = []){
		...
		$data['listComment'] = $this->comment_render($param['id'],$param['sort']);
		return $data;
	}
    public function statistical_rating($param = [])
    {
        
		
		//tính tổng số bản ghi có trong chi tiết module
        $totalRows =  ModelsComment::select('id','productid','rating')->where('productid',$param['productid'])->where('parentid',0)->where('publish',0)->get();
    ...
    public function comment_render($productid = 0,$sort = 'id'){
		//lấy toàn bộ comment
		$listComment = ModelsComment::select('comments.*')->where('productid',$productid)->where('parentid',0)->where('comments.publish',0);
		...
		if($sort == 'payment'){
			$listComment = $listComment->join('orders', 'orders.customerid', '=', 'comments.customerid')->join('orders_item', 'orders_item.order_id', '=', 'orders.id')->groupBy('comments.id')->where('product_id',$productid);

		}
		$listComment = $listComment->paginate(env('APP_paginate_comment'));
		if($listComment){
			foreach($listComment as $key => $val){
				$orderCheck = $val->order->where('product_id',$val->productid);
